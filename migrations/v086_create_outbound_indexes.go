...
// RunV086CreateOutboundIndexes applies the v086 migration.
//
...
func RunV086CreateOutboundIndexes(ctx context.Context, db *mongo.Database) error {
	slog.Info("running migration v086: ensure outbound collection indexes")
	...
	for _, step := range steps {
		if err := ensureOutboundIndexes(ctx, db, step.collection, step.indexes); err != nil {
			return fmt.Errorf("v086 ensure indexes for %s: %w", step.collection, err)
		}
	}
...
func ensureOutboundIndexes(ctx context.Context, db *mongo.Database, colName string, indexes []mongo.IndexModel) error {
	...
	for _, idx := range indexes {
		...
		slog.Info("v086: ensured index", "collection", colName, "index", name)
	}
	return nil
}
